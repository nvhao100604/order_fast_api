...
class User(Base):
    ...
    status: Mapped[Status] = mapped_column(
        SQLEnum(Status, name="user_status_enum"),
    ...
    phoneNumber: Mapped[str] = mapped_column(
        "phone_number", String(10), index=True, unique=True
    )
    address: Mapped[Optional[str]] = mapped_column(String(255))

    roleID: Mapped[Optional[int]] = mapped_column(
        "role_id", ForeignKey("roles.id")
    )
    internalRole: Mapped[Optional["Role"]] = relationship(
...
class Discount(Base):
    ...
    category: Mapped[DiscountCategory] = mapped_column(
        SQLEnum(DiscountCategory, name="discount_category_enum"),
        nullable=True,
    )
    ...
    status: Mapped[Status] = mapped_column(
        SQLEnum(Status, name="discount_status_enum"),
        default=Status.ACTIVE,
        server_default=Status.ACTIVE.value,
    )
